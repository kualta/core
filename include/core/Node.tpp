...
void Node<T>::SetParent(std::weak_ptr<T> newParent) {
    ...
    if ( !parent.expired() ) {
        parent.lock()->DeleteChild(std::static_pointer_cast<Entity>(this->shared_from_this()));
    }
    newParent.lock()->AddChild(std::static_pointer_cast<Entity>(this->shared_from_this()));
